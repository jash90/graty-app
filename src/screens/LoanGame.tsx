    ...
    return (
        <Box sx={{ display: 'flex', margin: 10, flexDirection: 'row' }} >
            <Box sx={{ display: 'flex', flex: 1, margin: 10, flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <Typography variant="h3" sx={{ alignSelf: 'center', fontWeight: 'bold' }}>Wypożycz Grę</Typography>
                <Autocomplete
                    disablePortal
                    options={list}
                    sx={{ width: 300, padding: 2 }}
                    renderInput={(params) => <TextField {...params} label="Gra planszowa" />}
                ...
                <Button variant="contained" sx={{ width: 300, margin: 2 }} type={"submit"}>
                    Wypożycz Grę
                </Button>
            </Box>
        </Box>
