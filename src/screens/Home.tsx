    ...
    return (
        <Box sx={{display: 'flex', margin: 10, flexDirection: 'row'}}>
            <Box sx={{display: 'flex', flex: 1, margin: 10, flexDirection: 'column', alignSelf: 'center'}}>
                ...
                <Typography variant="h4">Blogerzy, youtuberzy, streamerzy i organizatorzy eventów z grami planszowymi,
                    miejskimi oraz terenowymi.</Typography>
            </Box>
            ...
            <Box sx={{display: 'flex', flex: 1, margin: 10, flexDirection: 'column', alignSelf: 'center'}}>
                ...
                {isDevelopment && !user && <Box sx={{display: 'flex', flex: 1, margin: 10, flexDirection: 'row',}}>
                    <Button variant="contained" onClick={()=>setLoadingAndPassword('admin')}>
                        <Typography>Admin</Typography>
                    </Button>
                    <Button variant="contained" onClick={()=>setLoadingAndPassword('loaner')}>
                        <Typography>Loaner</Typography>
                    </Button>
                    <Button variant="contained" onClick={()=>setLoadingAndPassword('guest')}>
                ...
                {!!user && <Typography variant="h4" sx={{
                    alignSelf: 'center',
                    fontWeight: 'bold'
                }}>{`Witaj ${user.email.substring(0, user.email.indexOf("@"))}!`}</Typography>}
            </Box>
        </Box>
    );
}
